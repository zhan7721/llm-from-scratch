"""Data pipeline for LLM pretraining: datasets, collation, and packing."""
...
def dynamic_pad_collate(batch: List[Dict[str, torch.Tensor]], pad_id: int = 0) -> Dict[str, torch.Tensor]:
...
class PackedDataset(Dataset):
    ...
    Uses document boundaries to avoid cross-document attention.
    """
...
def create_pretrain_dataloader(
    ...
    """
    if use_packing:
        dataset = PackedDataset([token_ids], seq_len, pad_id)
    else:
        dataset = PretrainDataset(token_ids, seq_len)

    return DataLoader(
        dataset,
        batch_size=batch_size,
        shuffle=True,
        collate_fn=None if use_packing else lambda b: dynamic_pad_collate(b, pad_id),
        drop_last=True,
    )
